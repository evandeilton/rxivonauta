...
class RxivonautaPipeline:
    ...
    async def run(
        ...
        try:
            ...
            # Validate the model
            if not settings.OpenRouterConfig.validate_model(settings.OpenRouterConfig.MODEL):
                settings.OpenRouterConfig.MODEL = settings.OpenRouterConfig.DEFAULT_MODEL
                self.logger.warning(f"Invalid model specified. Using default model: {settings.OpenRouterConfig.DEFAULT_MODEL}")
